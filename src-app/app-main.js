...
function bootApp() {
    ...
    function attachHomeScreen(network = null) {
        ...
        if (screen.network.isConnected) {
            screen.bindNetworkHandlers();
            screen.statusMessage = t('connected');
            screen.network.requestCompetitiveProfile();
            screen.draw();
            return Promise.resolve();
        }

        return screen.connectToServer().then(() => {
            if (startScreen === screen && screen.isActive) screen.draw();
        });
    }

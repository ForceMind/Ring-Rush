...
- Token-protected admin page for user lookup, deletion, reset, and wallet adjustment.
...
The script installs dependencies, builds the website/game/admin pages with `VITE_PELLO_SERVER_URL=https://pello.xincreates.com`, stops the previous Pello service/processes from this project, finds a safe single Node port, generates or reuses an admin token, and writes a systemd service.
...
| `PELLO_ADMIN_TOKEN` | Token required by `/admin.html` and `/api/admin/*`. Generated by deploy script when omitted. |

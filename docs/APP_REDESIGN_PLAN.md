...
- Android now starts from `/app.html` through Capacitor `server.appStartPath`.
...
## Android App UI Addendum

The app UI is split from the online web game:

- `/app.html` imports `src-app/app-main.js`.
- `src-app` reuses the shared game logic but swaps in app-only start screen, HUD, board skin, input slider, and sprite-backed piece drawing.
- `public/assets/app-ui/atlas.png` and `manifest.json` are generated by `npm run app:ui-assets`.
- `design/app-ui/mockups` contains the effect mockups and visual references.
- The app can change visual assets without moving board constants, scoring zones, launch lanes, physics, or AI targeting coordinates.
...
- Split Android app UI from the browser game UI.
- Generate a real PNG app UI atlas and effect mockups.

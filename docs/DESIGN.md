...
- It shows paginated and categorized users with wallet, reserved coins, rating, record, queue state, recent ledger, and recent matches.
- It can bulk-delete no-record users only when they have no matches, no wins/losses, no queue or active match, no reserved coins, and no wallet/admin changes.

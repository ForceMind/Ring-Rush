...
export class AI {
    ...
    getRequiredStopSpeed(distance) {
        const tailDistance = SPEED_THRESHOLD / (1 - FRICTION);
        ...
        const addCandidate = (tx, ty, bank = false) => {
            for (let svIdx = 0; svIdx < searchConfig.sliderSteps; svIdx++) {
                ...
                for (const rawSpeed of speeds) {
                    ...
                    const key = `${svIdx}:${Math.round(angle * 1000)}:${Math.round(speed * 10)}`;
    ...
    selectLaunch(evaluatedLaunches, piece, game, baseProgress) {
        ...
        if (
            best.predictedScore === 5
            && Math.random() < decisionConfig.varietyPickRate
        ) {
    ...
    async calculateLaunch(piece, game) {
        ...
        let bestLaunch = {
            ...
            predictedScore: 0,
            runnerPositionAfter: game.runnerPosition
        };
        const evaluatedLaunches = [];

        const originalPieces = game.physics.pieces;
        const candidates = this.buildCandidates(game, piece);
        const baseProgress = this.getPlayerProgress(game.runnerPosition, piece.player);
        ...
        for (const candidate of candidates) {
            if (!this.isThinking) return bestLaunch;
